...
def pad_batch_data(insts,
    ...
    max_len = int(min(max_len,max_seq_len))
    base = []
    for inst in insts:
        if len(inst)>max_len:
            inst = inst[:max_len]
            ...
            inst = inst+ list([pad_idx] * (max_len - len(inst)))
            base.append(inst)
    # Any token included in dict can be used to pad, since the paddings' loss
    # will be masked out by weights and make no effect on parameter gradients.

    inst_data = np.array(base)
    ...
    if return_input_mask:
        ...
        for inst in insts:
            if len(inst) > max_len:
                inst = inst[:max_len]
            if len(inst) < max_len:
                mask_base.append([1] * len(inst) + [0] *(max_len - len(inst)))
            else:
                mask_base.append([1]*max_len)
        input_mask_data = np.expand_dims(np.array(mask_base), axis=-1)
