...
def init_pretraining_params(exe,
    ...
    def existed_params(var):
        if not isinstance(var, fluid.framework.Parameter):
            return False
        result = os.path.exists(os.path.join(pretraining_params_path, var.name))
        if result is True:
            print('load pretraining parameter {:s}'.format(var.name))
        return result

    print('-----------------------start initialize pretraining parameters from {:s}-----------------------'.format(pretraining_params_path))
    fluid.io.load_vars(
        exe,
        pretraining_params_path,
        main_program=main_program,
